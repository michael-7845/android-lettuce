...
def enable(filename=None):
    ...
    def create_test_case_step(step):
        ...
        #*-* mike add begin *-*
        def _read_logconf(filepath, feature_scenario_name):
            ...
            try:
                for line in file_object:
                    tag = line.split('@')[0].decode('utf-8')
                    path = line.split('@')[1].decode('utf-8')
                    if tag == feature_scenario_name:
                        logs.append(path)
            finally:
                file_object.close( )
            return logs
        
        logpath_rec = os.path.join(os.getcwd(), config.allog.dir, config.allog.logpath_record_file)
        logs = _read_logconf(logpath_rec, "%s:%s" % (parent.feature.name, name)) # name: scenario name
        if logs:
            doc.appendChild(tc)
            systemout = doc.createElement("system-out") 
            tc.appendChild(systemout) 
            try:
                for log in logs:
                    attachment = doc.createTextNode(log) 
                    systemout.appendChild(attachment)
            except:
                print logs
        #*-* mike add end *-* 
