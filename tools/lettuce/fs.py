...
class FeatureLoader(object):
    ...
    def find_and_load_step_definitions(self):
        ...
        while base_dir != '/':
            # *-* Kemin Modified *-*
            my_steps = FileSystem.locate(base_dir, '*.py')
            if my_steps:
                break
            base_dir = FileSystem.join(base_dir, '..')
       
        # *-* Kemin Add: Begin *-*
        global_dir = FileSystem.join(base_dir, '..', 'steps') 
        global_steps = FileSystem.locate(global_dir, '*.py')
        files = my_steps + global_steps
        # *-* Kemin Add: End *-*
        ...
        for filename in files:
            ...
            # *-* Kemin Add *-*
            if filename not in my_steps: # my_steps only import once
