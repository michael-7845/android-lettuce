...
class Adb(object):
    ...
    def cmd(self, *args):
        ...
        cmd_line = ["-s", ''.join(["\"",self.device_serial(),"\""])] + list(args)
    ...
    def devices(self):
        ...
        #kemim
        #print dict([s.split('\t') for s in out[index + len(match):].strip().splitlines() if s.strip()])
...
class AutomatorServer(object):
        ...
        if local_port:
            ...
            try:  # first we will try to use the local port already adb forwarded
                ...
                for s, lp, rp in self.adb.forward_list():
                    #如果serial和rp相等，那么就用这个lp进行通信
                    if s == self.adb.device_serial() and rp == 'tcp:%d' % self.device_port: 
                        self.local_port = int(lp[4:])
                        break
                else: #如果没有找到lp,那么新建立一个forward通信
                    self.local_port = next_local_port()
            except:
                self.local_port = next_local_port()

    #把jar包放到/data/local/tmp
    def push(self):
        base_dir = os.path.dirname(__file__) #本文件的目录近路
    ...
    def jsonrpc(self):
        ...
        # method,
        # timeout,
        # restart, True那么尝试重启Automator服务器
        def _JsonRPCMethod(url, method, timeout, restart=True):
            #kemin
            #print url, method, timeout
            _method_obj = JsonRPCMethod(url, method, timeout) #方法对象，JsonRPCMethod实例
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if restart:
                        # 停止后再启动，然后再次调用_JsonRPCMethod()，不再要求重启
                        server.stop()
                        server.start()
...
class AutomatorDevice(object):
    ...
    # 包含属性server, 为AutomatorServer实例（其中又包含adb实例）
    def __init__(self, serial=None, local_port=None):
        self.server = AutomatorServer(serial=serial, local_port=local_port)

    #AutomatorDevice实例是可调用的，d(**kwargs)实际上是返回的AutomatorDeviceObject的实例
    ...
    # dump windows布局文件，保存内容到本地的filename
    ...
    # 属性, open, 为1函数，参数可以为"notification", "quick_settings"
    # d.open.notification()
...
#AutomatorDeviceNamedUiObject，派生自AutomatorDeviceUiObject
# 作用：获取child和sibling功能？
...
#AutomatorDeviceObject，派生自AutomatorDeviceUiObject
# 通用的页面元素类，代表可以在上面执行操作的元素
# 1. AutomatorDeviceObject在d(**kwargs)时候返回此对象的实例，它是AutomatorDeviceUiObject子类, 所以也能执行AutomatorDeviceUiObject中的操作
# 2. child_by_xxx()系列函数，返回的是AutomatorDeviceNamedUiObject实例，它是AutomatorDeviceUiObject子类, 所以也能执行AutomatorDeviceUiObject中的操作
...
# device, AutomatorDevice使用缺省serial的实例
device = AutomatorDevice()
